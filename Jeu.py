...
class jeu:
    ...
    def __init__(self):
        ...
        win= Tk()
        win.focus_force()
        win.x= win.winfo_screenwidth()
        win.y= win.winfo_screenheight()
        win.geometry("{}x{}+{}+{}".format(win.x-20, win.y-50, 2, 5))
        ...
        global pays, find, labels, histo, ent_inp, nb
        pays= ''
        find= ''
        labels= ''
        ...
        def draw(action):
            ...
            if(action=='add'):
                ...
                photo= PhotoImage(file= "IHM\Pendu\\"+str(nb)+".gif")
                i_show_pendu.config(image= photo)
                nb+=1
        ...
        def yes(self=0):
            ...
            """
            if(self!=0):
                yesno= mb.askyesno("Perdu ...", "Et c' est dommage. Le mot etait {}.\nRecommencer ?".format(pays.upper()))          
        ...
        def checkMot(inp, choix):
            ...
            histo.append(inp)
            l_show_histo.config(text= str(histo))
        ...
        ##FRAME INPUT
        f_i= Frame(win)
        f_i.pack(expand=True)

        ent_inp= ttk.Entry(f_i)
        ent_inp.grid(row= 0, column=2, columnspan=4)

        Label(f_i, text='Voulez-vous donner une lettre ou un mot entier ?').grid(row=1, column=1, columnspan=6)
        ...
        b_l.grid(row= 2, column=3)

        b_m= Radiobutton(f_i, text='Mot entier', variable= b_sel, value= 1) #bouton selection mot
        b_m.grid(row= 2, column=4)


        ttk.Button(win, text= "Revenir au Menu Principal", command= launch_MP).place(x= 20, y=10)


        ##############


        ######Frame historique lettres######
        f_histo= Frame(win)
        f_histo.place(x= win.x - 300, y= win.y/3)

        Label(f_histo, text="Historique des lettres/mots tapées: \n").pack()
        l_show_histo= Label(f_histo)
        l_show_histo.pack(expand= True)
        ####################################
        ...
        ############Frame pendu##############
        f_pendu= Frame(win)
        f_pendu.place(x= win.winfo_width(), y= 100)


        global im
        photo= PhotoImage("IHM/Pendu/1.png")
        i_show_pendu= Label(f_pendu, image= photo)
        i_show_pendu.pack()
        #####################################
        ...
        label=Label(f_i, text="Pour des raisons de problème de caractères, certains pays sont écris en anglais")
        label.grid(row=4, column=1, columnspan=6)

        welcome=Label(f_i, text="Bienvenue sur le jeu du Pendu", font='Arial 14')
        welcome.grid(row=5, column=1, columnspan=6,pady=20)

        rule=Label(f_i, text="Pour vous rappeler des règles, faites un clique droit", font='Arial 14')
        rule.grid(row=6, column=1,columnspan=6)
