...
class Moyenne:
    ...
    def __init__(self):
        ...
        root.x= root.winfo_screenwidth()
        root.y= root.winfo_screenheight()
        root.geometry("{}x{}+{}+{}".format(root.x-20, root.y-50, 2, 5))
        ...
        def custom_ajout_mat(self=0):
            ...
            def checkValid_custom(event=0):
                if(type(ent.get()=="int")):
                    ajout_mat(int(ent.get()))
            ...
            ent.bind("<Return>", checkValid_custom)
        ...
        def commentaire(total_g, total_s, total_ie):
            ...
            if total_g>=8 and 6<=total_s<8 and total_ie>=8 and total_ie>total_s :
            ...
            if 8<total_g<10 and 6<total_s<=8 and 6<total_ie<=8 :
                l_text.config(text="Bah écoute: Bouge-toi le Callypige qu'est-ce que tu veux que je te dise !!!")
                return

            if 6<=total_g<8 and total_s<6 and total_ie<6 :
            ...
            if total_g<12 and total_g<total_s and total_g<total_ie and total_s>12 and total_ie>12 :
                l_text.config(text="Les langues, la P5 et la MSH semblent être une vaste contrée négative pour toi ...")
                return
            
            if total_g>=8 and total_ie<7 and total_s>=8 :
            ...
            if total_g>=10:
                if 9<=total_s<10:
                    if 10>total_ie>=9 :    
                        ...
                        return        
                    if 9>total_ie>=6 :
                        l_text.config(text="Va falloir penser à enclencher la deuxième hein ?")
                        return
                ...
                if 10>total_ie>=9 :                    
                    ...
                    if 6<=total_s<9 :
                        l_text.config(text="Va falloir penser à enclencher la deuxième hein ?")
                        return
            if 10>total_g>=9 and 10>total_s>=8 and 10>total_ie>=8 :
                l_text.config(text="Va falloir penser à enclencher la deuxième hein ?")
                return
            ...
            if 12>total_g>=11 and 11>total_s>10 and 11>total_ie>10 :
                l_text.config(text="Peut mieux faire")
                return

            if 15>total_g>=12 and 15>total_s>=11 and 15>total_ie>=11 :
